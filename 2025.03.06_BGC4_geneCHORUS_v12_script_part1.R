...
Dir.Name <- "BGC4_geneCHORUS_v12_pt1"
dir.create(paste0("/u/project/kp1/jlangerm/Projects/Gomperts_IPF/Chandani/Analysis/",Sys.Date(),"_", Dir.Name))
outdir <- c(paste0("/u/project/kp1/jlangerm/Projects/Gomperts_IPF/Chandani/Analysis/",Sys.Date(),"_", Dir.Name,"/"))

Exp.Name <- "BGC4"


Object_data<- readRDS("/u/project/kp1/jlangerm/Projects/Gomperts_IPF/Chandani/Analysis/2024-10-01_BGC4_CellCallFix/BGC4.Merge_Corrected.object.obj")

normalized.data <-as.data.frame(Object_data@assays$RNA$data)

markers<- read.table("/u/project/kp1/jlangerm/Projects/Gomperts_IPF/Chandani/Analysis/2024-10-01_BGC4_CellCallFix/BGC4.Aggrecluster.metadata.txt")


##Recovery
# markers<- read.table("/u/project/kp1/jlangerm/Projects/Gomperts_IPF/Chandani/Analysis/2025-03-07_BGC4_geneCHORUS_v12_pt1/BGC4.OverKClusters.metadata.txt")
# variance.tag <- read.table("/u/project/kp1/jlangerm/Projects/Gomperts_IPF/Chandani/Analysis/2025-03-07_BGC4_geneCHORUS_v12_pt1/BGC4.vox_corr_table.step6_all.gene.table.txt")
...
num.of.cells <- 100
markers$Cluster <- 0
markers$Cluster <- as.factor(kmeans(markers[,c("X","Y")], iter.max=30, nstart=5, centers=round(nrow(markers)/num.of.cells))$cluster) 
...
plot1<-ggplot(markers)+
  ...
  geom_text(aes(meanX,meanY, label=Cluster), size=5)+
...
ggsave(plot=plot1,filename=paste0(outdir,  Exp.Name, ".OverKClusters.labeled_nice.UMAP.png"),
       height=15, width=18)
write.table(markers, paste0(outdir,Exp.Name,".OverKClusters.metadata.txt"),
            sep="\t", quote=F)
...
## C87
##This is just if you want to look at results
# min(sized.sample.df[,"C87"])
# new.sample.df2 <-sized.sample.df
# new.sample.df2[new.sample.df2==0]<-1000
# checker.min2 <- cbind.data.frame(Cell=colnames(new.sample.df2), ColMins=apply(new.sample.df2,2,min))
...
### Rechair these 7 billion things
